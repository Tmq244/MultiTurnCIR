...
class FashionIQDataset(data.Dataset):
    ...
    def __load_pil_image__(self, path):
        try:
            ...
            print(err)
            img = Image.new('RGB', (224, 224)) # create a new blank 224*224 RGB image
            return img
    ...
    # Clean the raw tags into a two-dimensional list with a fixed length (size)
    def _normalize_tag_groups(self, tags, size):
...
class FashionIQTrainValDataset(FashionIQDataset):
    ...
    def __load_data__(self):
        with open('text_embedding.pkl', 'rb') as f:
            self.we = pickle.load(f)
        with open('image_embedding.pkl', 'rb') as f:
        ...
        for t in targets:
            ...
            full_tag_path = os.path.join('attr', tag_file)
            print(full_cap_path)
            ...
            with open(full_cap_path,'r') as f:
                ...
                for i,d in enumerate(tqdm(data)):
                    ...
                    for references in ref_list:
                        c_id = references[2]
                        caption = references[1] # the modified texts
                        if c_id in tags_dict:
                            # the first tag is the type of target like dress, tee
                            tags = self._normalize_tag_groups(tags_dict[c_id][1:], 5)
                        else:
                            tags = self._normalize_tag_groups([], 5)
                        ...
                        if self.split == 'train':
                            random.shuffle(text) # Only shuffle the order in training set
                    ...
                    if t_id in tags_dict:
                        target_tag = self._normalize_tag_groups(tags_dict[t_id], 6)
                    else:
                        target_tag = self._normalize_tag_groups([], 6)
                    target_tag_list = []
...
class FashionIQTestDataset(FashionIQDataset):
    ...
    def __load_data__(self):
        ...
        self.index_dataset = []
        with open(os.path.join('image_splits', split_file), 'rb') as f:
        ...
        for id in index_id:
            if id not in tags_dict:
                tags_dict[id] = self._normalize_tag_groups([], 6)
            _data = {
                'img_path': os.path.join(self.image_root, 'images/{}.jpg'.format(id)),
                'img_id': id,
                'img_tag': self._normalize_tag_groups(tags_dict[id], 6)
            }
